...
# --- Load modules and pin compilers WITHOUT hard-coding paths ---
if command -v module >/dev/null 2>&1; then
  log "Loading cluster modules"
  # Try CUDA 12.1 first, then 11.8 as fallback
  ...
  # Ensure GCC 11.x
  module load GCC/11.3.0 2>/dev/null || module load GCCcore/11.3.0 2>/dev/null || true
fi
...
if [[ -z "${CC}" || -z "${CXX}" ]]; then
  echo "Error: gcc/g++ not found on PATH. Load a GCC 11 module first."
  exit 1
fi

GCC_MAJOR="$("$CC" -dumpversion | cut -d. -f1 || echo "")"
if [[ "$GCC_MAJOR" != "11" ]]; then
  echo "Error: GCC 11.x required, but found: $("$CC" -dumpfullversion 2>/dev/null || "$CC" -dumpversion)"
  echo "Tip: run 'module avail GCC' and 'module load GCC/11.3.0' (or GCCcore/11.3.0)."
  exit 1
fi
...
# --- Install Python deps in a robust order ---
log "Installing conda and pip dependencies"
conda install -y pip nb_conda_kernels -y
...
# fixed commit to avoid known bug; no build isolation to respect CC/CXX
pip install --no-build-isolation git+https://github.com/drprojects/point_geometric_features.git@4102aa9
